...
public class BankApplication {
	...
	private static void createAccount() {
		...
		/*
		 * 참고: String이 참조 타입이기 때문에 아래와 같이 사용하는 것 처럼 Account도 참조 타입이다. String[] arr =
		 * {"a", "b"};
		 * 
	...
	private static void accountList() {
		...
		for (Account account : accountArray) {
			// 배열이 null 값일때 실행하는걸 방지하기 위해
			if (account != null) {
				System.out.print(account.getAno());
				System.out.print("  ");
				System.out.print(account.getOwner());
				System.out.print("  ");
				System.out.print(account.getBalance() + "입금액");
				System.out.println("  ");
			}
		}
		/*
		 * // * 힌트: for(Account account : accountArray) { System.out.println(account); }
		 */
	}
	...
	private static void deposit() {
		...
		System.out.println("예금액: ");
		int money = scanner.nextInt(); // int 타입으로 입력을 받는다.
		...
		for (Account account : accountArray) {
			...
			if (account != null) {
				if (ano.equals(account.getAno())) {
					// 여기서 return 하면 안되므로 아래와 같이 찾은계좌를 account타입의 변수인 resultAccount에 넣어준다
